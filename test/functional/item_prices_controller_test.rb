require 'test_helper'

class ItemPricesControllerTest < ActionController::TestCase
  setup do
    @item_price = item_prices(:one)
  end

  test "should get index" do
    get :index
    assert_response :success
    assert_not_nil assigns(:item_prices)
  end

  test "should get new" do
    get :new
    assert_response :success
  end

  test "should create item_price" do
    assert_difference('ItemPrice.count') do
      post :create, item_price: {  }
    end

    assert_redirected_to item_price_path(assigns(:item_price))
  end

  test "should show item_price" do
    get :show, id: @item_price
    assert_response :success
  end

  test "should get edit" do
    get :edit, id: @item_price
    assert_response :success
  end

  test "should update item_price" do
    put :update, id: @item_price, item_price: {  }
    assert_redirected_to item_price_path(assigns(:item_price))
  end

  test "should destroy item_price" do
    assert_difference('ItemPrice.count', -1) do
      delete :destroy, id: @item_price
    end

    assert_redirected_to item_prices_path
  end
end
